...
class ArtifactsTableViewController: UITableViewController {
    ...
    @IBOutlet var topContainer: CorneredView!
    @IBOutlet var bottomContainer: CorneredView!
    ...
    private func downloadArtifact(artifact: Artifact, completion: @escaping (Data?) -> Void) {
        guard let account = account
        else { return }
        currentDownloadTask = NetworkManager.manager.downloadArtifact(artifact: artifact, account: account) { data, error in
            DispatchQueue.main.async {
    ...
    private func showErrorMessage(for error: Error, with artifact: Artifact, completion: @escaping (Data?) -> Void) {
        displayNetworkError(error: error, onReturnWithTextFields: { dict in
    ...
    private func shareArtifact(data: Data, artifact: Artifact) {
        ...
        do {
            ...
            displayError(title: "Error", message: "Could not save file",
