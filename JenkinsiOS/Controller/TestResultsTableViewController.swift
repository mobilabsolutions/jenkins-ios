...
class TestResultsTableViewController: RefreshingTableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: searchImage, landscapeImagePhone: searchImage, style: .plain, target: self, action: #selector(search))
    ...
    private func fillUI() {
        ...
        self.tableView.contentOffset.y = self.searchBarContainer.frame.height - (navigationController?.navigationBar.frame.height ?? 0) - UIApplication.shared.statusBarFrame.height
    }
    ...
    private func updateCases() {
        ...
        self.cases = suites.flatMap({ $0.cases }).filter { $0.status == nil || currentScope.equals(status: $0.status!) }
    }
