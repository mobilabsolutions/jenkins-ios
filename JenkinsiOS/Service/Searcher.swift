...
class Searcher: NSObject {
    ...
    var additionalSearchCondition: ((Searchable, String?) -> Bool)?
    ...
    func searchAndFilter(searchString: String, scope: String? = nil) -> [Searchable] {
        ...
        if includeAllOnEmptySearchString && searchString.isEmpty {
            return searchableData.filter({ (searchable) -> Bool in
                additionalSearchCondition?(searchable, scope) ?? true
            })
        }

        return searchableData.filter({ (searchable) -> Bool in
            searchable.lowerCasedSearchString.contains(searchString) && (additionalSearchCondition?(searchable, scope) ?? true)
